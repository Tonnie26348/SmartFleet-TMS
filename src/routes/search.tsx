    ...
    <div className="min-h-screen bg-secondary/30">
      <header className="border-b border-border bg-background">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
          ...
          <Button asChild variant="ghost" size="sm"><Link to="/">New search</Link></Button>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold sm:text-3xl">
            {from || "Anywhere"} <ArrowRight className="inline h-6 w-6 text-muted-foreground" /> {to || "Anywhere"}
          </h1>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                    <span className="flex items-center gap-1"><Clock className="h-4 w-4" /> {formatDateTime(t.departure_at)}</span>
                    <span>·</span>
                    <span>{t.vehicle?.model} · {t.vehicle?.capacity} seats</span>
                    <span>·</span>
                ...
                <div className="flex items-center justify-between gap-4 sm:flex-col sm:items-end">
                  ...
                  <Button asChild>
                    <Link to="/trips/$tripId" params={{ tripId: t.id }}>Select seats <ArrowRight className="h-4 w-4" /></Link>
                  </Button>
                </div>
              </div>
            </Card>
